...
/*
  ...
  */
    ...
    public void scoresjc(){
        ...
        String[] strcs = {"sj"};
        String[] cs =  dbReader.readDB3(strcs, "course.sjclass=? AND course.scode = score.scode AND score.score !=? AND score.score !=?", new String[]{"人文・社会科学等科目", "F", ""},"course, score").split("\n");
        String[] cs2 = dbReader.readDB3(strcs, "course.sjclass=? AND course.scode = score.scode AND score.score !=? AND score.score !=?" , new String[]{"自然科学等科目", "F",""},"course, score").split("\n");
        String[] cs3 = dbReader.readDB3(strcs, "course.sjclass=? AND course.scode = score.scode AND score.score !=? AND score.score !=?" , new String[]{"専門科目", "F", ""},"course,score").split("\n");
        String[] cs4 = dbReader.readDB3(strcs, "course.sjclasssub=? AND course.scode = score.scode AND score.score !=? AND score.score !=?" , new String[]{"工学系共通科目", "F", ""},"course, score").split("\n");
        String[] cs5 = dbReader.readDB3(strcs, "course.sjclasssub=? AND course.scode = score.scode AND score.score !=? AND score.score !=?" , new String[]{"専門基礎科目", "F",""},"course,score").split("\n");

        String[] cs6 = dbReader.readDB3(strcs, "course.sjclasssub=? AND course.scode = score.scode AND score.score !=? AND score.score !=?" , new String[]{"専門発展科目", "F", ""},"course, score").split("\n");
        String[] cs7 = dbReader.readDB3(strcs, "course.sjclasssub=? AND course.scode = score.scode AND score.score !=? AND score.score !=?" , new String[]{"専攻領域科目", "F", ""},"course, score").split("\n");
        String[] cs8 = dbReader.readDB3(strcs, "course.sjclasssub=? AND course.scode = score.scode AND score.score !=? AND score.score !=?" , new String[]{"他学群・他学部専門科目", "F", ""},"course,score").split("\n");
